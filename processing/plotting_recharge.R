...
df_in_R <- read_csv("inputs/inputs_recharge_lakes.csv")
sf_in_R <- st_read("inputs/shapefiles/inputs_recharge_plus_lakes.shp") %>% st_make_valid()
colnames(sf_in_R) <- c(colnames(df_in_R), "geometry")

mappings_all <- read_csv("processing/basin_country_region_mapping.csv")

figs_dir <- "processing/figures/withRecharge/"
out_dir <- "outputs/superwell_py_deep_C_all_B_all_G_all_"

# main scenario, to load lesser rows n_max = 1000
df_0.3PD_0.25DL <- read_csv(paste0(out_dir, '0.3PD_0.25DL_0.2RR.csv')) %>% rename("GridCellID" = "grid_id")
...
{
df_0.3PD_0.05DL <- read_csv(paste0(out_dir, '0.3PD_0.05DL_0.2RR.csv')) %>% rename("GridCellID" = "grid_id")
df_0.3PD_0.4DL <- read_csv(paste0(out_dir, '0.3PD_0.4DL_0.2RR.csv')) %>% rename("GridCellID" = "grid_id")
df_0.6PD_0.05DL <- read_csv(paste0(out_dir, '0.6PD_0.05DL_0.2RR.csv')) %>% rename("GridCellID" = "grid_id")
df_0.6PD_0.25DL <- read_csv(paste0(out_dir, '0.6PD_0.25DL_0.2RR.csv')) %>% rename("GridCellID" = "grid_id")
df_0.6PD_0.4DL <- read_csv(paste0(out_dir, '0.6PD_0.4DL_0.2RR.csv')) %>% rename("GridCellID" = "grid_id")
}
...
# output data ----
df_0.3PD_0.25DL %>% left_join(select(sf_in_R, c("GridCellID", "geometry")), by = "GridCellID") -> df_out

# clean data

# # determine unique combinations of country and GridCellID in df and in df_in_R
# df %>% as.data.frame() %>% select(COUNTRY, OriginalOb) %>% unique() -> df_unique
# df_in_R %>% select(CNTRY_NAME, OBJECTID) %>% unique() -> df_in_unique
# # determine the difference in both unique dataframes df and df_in_R
...
df_in_R %>%  mutate(Aquifer_thickness = case_when(Aquifer_thickness > 1000 ~ 1000,
...
get_scen <- function(x) eval(as.name(x), envir = .GlobalEnv)
...
calculate_stats <- function(scen_list) {
  ...
  for (scenario in scen_names_list) {
    ...
    df <- get_scen(paste0("df_", scenario))
...
# round everything but "scenario" columns in in fractions to 3 decimal places
fractions[, -1] <- round(fractions[, -1], 3)
print(fractions)


# write fraction as a csv
write.csv(fractions, "outputs/fractions_R.csv", row.names = FALSE)
...
ggplot(global_timeseries) +
  geom_line(aes(x = year_number, y = global_vol_prod * 1e-3, color = scenario, linetype = scenario), linewidth = 1) +
...
ggplot(global_timeseries %>%
         group_by(scenario) %>% mutate(global_vol_prod_cumsum = cumsum(global_vol_prod) * 1e-6)) +
  geom_line(aes(x = year_number, y = global_vol_prod_cumsum, color = scenario, linetype = scenario), linewidth = 1) +
...
ggplot(global_timeseries) +
  geom_line(aes(x = year_number, y = energy2nonenergy_ratio, color = scenario, linetype = scenario), linewidth = 1) +
...
ggplot(global_timeseries) +
  geom_line(aes(x = year_number, y = global_total_cost * 1e-9, color = scenario, linetype = scenario), linewidth = 1) +
...
ggplot(global_timeseries) +
  geom_line(aes(x = year_number, y = global_unit_cost, color = scenario, linetype = scenario), linewidth = 1) +
...
ggplot(global_timeseries %>%
         group_by(scenario) %>% mutate(global_unit_cost_cumsum = cumsum(global_unit_cost))) +
  geom_line(aes(x = year_number, y = global_unit_cost_cumsum, color = scenario, linetype = scenario), linewidth = 1) +
...
df_0.3PD_0.25DL %>%
  select(year_number, annual_capital_cost, maintenance_cost, energy_cost) %>%
  mutate(annual_capital_cost = annual_capital_cost * 1e-12,
         maintenance_cost = maintenance_cost * 1e-12,
         energy_cost = energy_cost * 1e-12) %>%
  group_by(year_number) %>% summarise_all(sum) %>% ungroup() %>% mutate(scen = "0.3PD_0.25DL") %>%
  ...
  labs(x = 'Pumping Year', y = expression('Global Annual Cost (Trillion $)'), fill = "Cost Type", tag = "(a)") +
  scale_fill_manual(values = c("#00b0f0", "#f4aa00", "#c10435")) + my_theme() +
  theme(legend.position = c(0.825, 0.85),
        legend.title = element_text(face = "bold"))

ggsave(paste0(figs_dir, "global_annual_cost_total_0.3PD_0.25DL.png"), width = 6, height = 4, units = "in", dpi = 300)

df_0.3PD_0.25DL %>%
  select(year_number, annual_capital_cost, maintenance_cost, energy_cost) %>%
  mutate(annual_capital_cost = annual_capital_cost * 1e-6,
         maintenance_cost = maintenance_cost * 1e-6,
         energy_cost = energy_cost * 1e-6) %>%
  ...
  ggplot() +
  geom_bar(aes(fill = cost_type, y = cost, x = year_number), position = "stack", stat = "identity") +
  labs(x = 'Pumping Year', y = expression('Global Average Annual Cost (Million $)'), fill = "Cost Type", tag = "(b)") +
  scale_fill_manual(values = c("#00b0f0", "#f4aa00", "#c10435")) + my_theme() +
  theme(legend.position = c(0.825, 0.85),
        legend.title = element_text(face = "bold"))

ggsave(paste0(figs_dir, "global_annual_cost_avg_0.3PD_0.25DL.png"), width = 6, height = 4, units = "in", dpi = 300)
...
ggsave(paste0(figs_dir, "global_annual_cost_avgperwell_0.3PD_0.25DL.png"), width = 6, height = 4, units = "in", dpi = 300)
...
plot_single_well <- function(data, x_var = "year_number", y_var, y_scale = 1, y_label, color = "red", tag = "") {
  ...
  # list to plot everything in one plot
  plot_list[[y_var]] <<- gg
...
# combined plot of single cell dynamics
combined_plot <-patchwork:: wrap_plots(plot_list, ncol = 6)
# maintain 16:9 ratio but keep the height to 12
ggsave(paste0(figs_dir, "single_well_dynamics.png"), combined_plot, width = 24.88889, height = 14, units = "in", dpi = 300)
...
plot_input <- sf_in_R %>% # filter(Country == "United States") %>%
  mutate(Grid_area_km = Grid_area/10^6, # convert to km2
         Lake_area_km = Lake_area/10^6, # convert to km2
  ...
  select(c("Porosity", "Permeability", "Aquifer_thickness",
           "Depth_to_water", "Grid_area_km", "Lake_area_km", "Recharge", "WHYClass")) # filter only relevant ones
...
legendnames <- c("Porosity" = "Porosity (-)", "Permeability" = expression('Permeability (m'^2*')'),
                 ...
                 "Grid_area_km" = expression('Grid Area (km'^2*')'),
                 "Lake_area_km" = expression('Lake Area (km'^2*')'),
                 "Recharge" = "Recharge (m/yr)", "WHYClass" = "WHY Class") # to label legend properly
# see all palettes available: tmaptools::palette_explorer()
pal <- c("Porosity" = "BuPu", "Permeability" = "YlGn",
         "Aquifer_thickness" = "YlOrRd", "Depth_to_water" = "YlOrBr",
         "Grid_area_km" = "BuGn", "Lake_area_km" = "YlGnBu",
         "Recharge" = "Blues", "WHYClass" = "PuBuGn")

# max(plot_input$Permeability)

# plot all input variables using ggplot
for (plot in colnames(plot_input)[4:(length(plot_input) - 5)]) { # plot all input variables except last two
  ggplot(plot_input) +
...
ggplot(plot_input) +
  ...
  scale_fill_gradientn(name = "Recharge (m/yr)",
                       ...
                       values = scales::rescale(sort(c(min(plot_input$Recharge),
                                                       kmeans(plot_input$Recharge, centers = 2)$center, # breaks by kmeans clustering
                                                       max(plot_input$Recharge))))
  ) +
  coord_sf(expand = FALSE) + # makes sure the plot doesn't extend beyond Earth (also plots y axis ticks and crops)
  theme_minimal() + theme(plot.margin = grid::unit(c(-4, 0, -4, 0), "cm"),
                          legend.justification = c(0,0), legend.position = c(0,0),
...
ggsave(paste0(figs_dir, "map_in_Recharge_sep.png"), width = 11, height = 3.5, units = "in", dpi = 300)

# for lake area
ggplot(plot_input) +
  geom_sf(lwd = 0, aes(fill =  Lake_area_km), lwd = 0, colour = NA) +
  scale_fill_gradientn(name = expression('Lake Area (km'^2*')'),
                       colors = c(brewer.pal(9, "GnBu")),
                       values = scales::rescale(sort(c(min(plot_input$Lake_area_km),
                                                       kmeans(plot_input$Lake_area_km, centers = 2)$center, # breaks by kmeans clustering
                                                       max(plot_input$Lake_area_km))))
  ) +
  coord_sf(expand = FALSE) + # makes sure the plot doesn't extend beyond Earth (also plots y axis ticks and crops)
  theme_minimal() + theme(plot.margin = grid::unit(c(-4, 0, -4, 0), "cm"),
                          legend.justification = c(0,0), legend.position = c(0,0),
...
ggsave(paste0(figs_dir, "map_in_Lake_area_km_sep.png"), width = 11, height = 3.5, units = "in", dpi = 300)


# plot adjusted grid area from the outputs not the input
lakes_affected %>% select(GridCellID, grid_area) %>% unique() %>%
  mutate(adjusted_grid_area_km = grid_area * 1e-6) %>%
  add_geometry() -> lakes_affected_plot

  ggplot(lakes_affected_plot) +
  geom_sf(aes(fill = adjusted_grid_area_km), lwd = 0, colour = NA) +
  scale_fill_gradientn(name = expression('Adjusted Grid Area\n(km'^2*')'),
                       colors = c(brewer.pal(9, "BuGn")),
                       values = scales::rescale(sort(c(min(lakes_affected_plot$adjusted_grid_area_km),
                                                       kmeans(lakes_affected_plot$adjusted_grid_area_km, centers = 7)$center, # breaks by kmeans clustering
                                                       max(lakes_affected_plot$adjusted_grid_area_km))))
  ) +
  coord_sf(expand = FALSE) + # makes sure the plot doesn't extend beyond Earth (also plots y axis ticks and crops)
  theme_minimal() + theme(plot.margin = grid::unit(c(-4, 0, -4, 0), "cm"),
                          legend.justification = c(0,0), legend.position = c(0,0),
...
ggsave(paste0(figs_dir, "map_out_adjusted_grid_area_km.png"), width = 11, height = 3.5, units = "in", dpi = 300)
...
plot_map <- function (plot_data, plot_var, legend_var, color_var, kcenter = 7) {
  # plot <-
  ggplot(plot_data) +
  ...
  scale_fill_gradientn(name = legend_var,
                       ...
                       values = scales::rescale(sort(c(min(plot_data[[plot_var]]),
                                                       kmeans(plot_data[[plot_var]], centers = kcenter)$center, # breaks by kmeans clustering
...
paste0("Cells where all of shallow recharge depth is used to reduce ponded depth target: ",
       length(unique((df_out %>% select(c("GridCellID", shallow_recharge_depth, threshold_depth)) %>%
                        filter(shallow_recharge_depth <= threshold_depth))$GridCellID)))


df_out_recharge <- df_out %>% select(c("GridCellID", recharge_vars, volume_produced_allwells)) %>%
  # filter(GridCellID == 72548) %>%
  group_by(GridCellID) %>%
...
plot_map(df_out_recharge, "recharge", "Recharge (m)", "Blues", 2)
plot_map(df_out_recharge, "net_ponded_depth_target", "Net Ponded Depth Target (m)", "PuBu")
plot_map(df_out_recharge, "shallow_recharge_depth", "Shallow Recharge\nDepth (m)", "Greens", 2)
plot_map(df_out_recharge, "shallow_recharge_vol", "Shallow Recharge\nVolume (km3)", "Greens", 2)
...
## cutoff cost vs vol at a cost cutoff map ----
...
# analysis for cost numbers: based on 25% 0.3 scenario (full analysis needs to happen with df_unit to get all scenarios)
median(df_out$unit_cost)
describe(df_out$unit_cost)
describe((df_out %>% mutate(volume_produced_allwells = volume_produced_allwells * 1e-9))$volume_produced_allwells)
...
# df_scatter <- read_csv(paste0(out_dir, '0.3PD_0.25DL_sample_100.csv')) %>% rename("GridCellID" = "grid_id") %>% left_join(select(sf_in_R, c("GridCellID", "geometry")), by = "GridCellID")
...
print(paste0("Number of Grid Cells for Scatter Plots: ", length(unique(df_scatter$GridCellID)),
             " out of ", length(unique(df_in_R$GridCellID)),
             " (", round(length(unique(df_scatter$GridCellID))*100/length(unique(df_in_R$GridCellID)), 1), "%)"))
...
df_scatter %>% select(c("GridCellID", "areal_extent", "well_yield", "net_ponded_depth_target")) %>%
  group_by(GridCellID) %>% unique() %>%
  mutate(plength = nrow(.)) %>% ungroup() %>%
  ggplot(aes(x = areal_extent * 1e-6, y = well_yield * 86400)) +
  geom_point(aes(color = net_ponded_depth_target), linewidth = 1.95) + # , alpha = 0.5
  # geom_point(linewidth = 1.95) +
  # geom_point(aes(size = well_yield * 86400)) +
  ...
  # scale_y_continuous(sec.axis = sec_axis(~.x * 0.1835, name = "Well Yield [gallons/min]")) +
  geom_text(aes(label = paste0("n = ", plength)),
            x = Inf, y = -Inf, hjust = 1.7, vjust = -17,
            size = 4, fontface = "italic") +
  labs(x = expression("Well Area (km"^2*")"), y = expression('Well Yield (m'^3*'/day)'), color = "Net Ponded\nDepth Target", tag = "(b)") +
...
# for numbers related to unit cost and volume produced (maybe for cutoffs as well)
# NOTE: BETTER ANALYSIS IS IN THE NEXT SECTION AFTER PLOTTING
# this is across all 3 depletion limits with 0.3PD
c(min(df_unit$unit_cost), max(df_unit$unit_cost))
describe(df_unit$unit_cost)
describe((df_unit %>% mutate(volume_produced_allwells = volume_produced_allwells * 1e-9))$volume_produced_allwells) # in km3 (no visible change from 25% DL, I guess becasue the pumping volume target is the same)

df_unit %>% group_by(depletion_limit) %>% summarise(min = min(unit_cost), max = max(unit_cost), mean = mean(unit_cost), median = median(unit_cost), sd = sd(unit_cost))
df_unit %>% mutate(volume_produced_allwells = volume_produced_allwells * 1e-9) %>% group_by(depletion_limit) %>%
  summarise(min = min(volume_produced_allwells), max = max(volume_produced_allwells), mean = mean(volume_produced_allwells), median = median(volume_produced_allwells), sd = sd(volume_produced_allwells))

# volume produced under 90th percentile of unit cost (* 1e-12)
# df_out %>% summarise(volume_produced_90th = sum(volume_produced_allwells[unit_cost <= quantile(unit_cost, 0.9)]) * 1e-12) # just to check the line below
df_unit %>% group_by(depletion_limit) %>% summarise(volume_produced_90th = sum(volume_produced_allwells[unit_cost <= quantile(unit_cost, 0.9)]) * 1e-12)


# unit cost value for which 90% of volume could be produced at a lower cost
# df_out %>% summarise(unit_cost_90th = quantile(unit_cost[volume_produced_allwells <= quantile(volume_produced_allwells, 0.9)], 1))
df_unit %>% group_by(depletion_limit) %>% summarise(unit_cost_90th = quantile(unit_cost[volume_produced_allwells <= quantile(volume_produced_allwells, 0.9)], 0.9))




df_out %>% select(c("GridCellID", "year_number", "depletion_limit",
                    "volume_produced_allwells", "unit_cost"))
...
describe(df_unit_bin_global$unit_cost_bin)
describe(df_unit_bin_global$volume_produced_allwells_bin)
...
sf_in_R %>% select(GridCellID, GCAM_basin_ID, Continent, Country, Basin_long_name) %>%
  left_join(mappings_all %>%
              filter(region == "Africa_Northern") %>%
...
unit_cost_scale_map <- ggplot() +
  ...
  geom_sf(data = sf_in_R, fill = "grey85", color = "grey85", size = 0.25) + # global
  geom_sf(data = scale_map_region, fill = "red4", color = "red4") + # region
  geom_sf(data = sf_in_R %>% filter(Continent == "Au"), fill = "gold2", color = "gold2") + # continent
  geom_sf(data = sf_in_R %>% filter(Country == "United States"), fill = "darkblue", color = "darkblue") + # country
  geom_sf(data = sf_in_R %>% filter(Basin_long_name == "Indus"), fill = "darkgreen", color = "darkgreen") + # basin
  # geom_sf(data = sf_in_R %>% filter(GridCellID %in% c("12429")), fill = "black", color = "black", size = 10) +
  geom_sf(data = sf_in_R %>% filter(GridCellID == "12416"), fill = "black", color = "black", size = 10) + # grid, full area
  coord_sf(expand = FALSE) + # makes sure the plot doesn't extend beyond Earth (also plots y axis ticks and crops)
  theme_minimal() + theme(panel.grid = element_blank(),
                          plot.margin = grid::unit(c(-4, 0, -4, 0), "cm"),
...
# cost and volume analysis global ----
# most frequent unit cost bin (mode)
df_unit_bin_global_cumsum %>% mutate(unit_cost_bin = round(unit_cost_bin, 2)) %>%
  count(unit_cost_bin) %>%
  filter(n == max(n)) %>%
  knitr::kable(caption = "Most frequent unit cost bin globally")

# the unit cost bin with most volume pumped
df_unit_bin_global_cumsum %>%
  filter(volume_produced_allwells_bin == max(volume_produced_allwells_bin)) %>%
  select(unit_cost_bin, volume_produced_allwells_bin) %>%
  mutate(volume_produced_allwells_bin = round(volume_produced_allwells_bin, 2)) %>%
  knitr::kable(caption = "Unit cost bin with most volume produced globally")
...
describe(df_unit_bin_global_cumsum)
describe(df_unit_bin_global_cumsum$unit_cost_bin)
describe(df_unit_bin_global_cumsum$volume_produced_allwells_bin)

## summary vol and unit cost stats: unit cost at x% volume produced ----
df_unit_bin_global_cumsum %>%
  summarise(total_volume = max(cumm_volume_produced_allwells_bin),
            unit_cost_10th = unit_cost_bin[which.min(abs(cumm_volume_produced_allwells_bin - 0.1 * total_volume))],
            unit_cost_50th = unit_cost_bin[which.min(abs(cumm_volume_produced_allwells_bin - 0.5 * total_volume))],
            unit_cost_75th = unit_cost_bin[which.min(abs(cumm_volume_produced_allwells_bin - 0.75 * total_volume))],
            unit_cost_90th = unit_cost_bin[which.min(abs(cumm_volume_produced_allwells_bin - 0.9 * total_volume))])

# average across scenarios (this table feeds the abstract of superwell GMD paper)
df_unit_bin_global_cumsum %>%
  summarise(total_volume = max(cumm_volume_produced_allwells_bin),
            unit_cost_10th = unit_cost_bin[which.min(abs(cumm_volume_produced_allwells_bin - 0.1 * total_volume))],
            unit_cost_50th = unit_cost_bin[which.min(abs(cumm_volume_produced_allwells_bin - 0.5 * total_volume))],
            ...
            unit_cost_90th = unit_cost_bin[which.min(abs(cumm_volume_produced_allwells_bin - 0.9 * total_volume))]) %>%
  ungroup() %>%
  mutate(depletion_limit = as.numeric(as.character(depletion_limit))) %>%
  summarise(across(everything(), mean))


# volume produced at x% unit cost
## cost percentiles ----
df_unit_bin_global_cumsum %>%
  group_by(depletion_limit) %>%
  summarise(total_volume = max(cumm_volume_produced_allwells_bin),
            # calculate the 10th, 50th, and 90th percentiles of unit cost
            unit_cost_10th = quantile(unit_cost_bin, 0.10),
            unit_cost_50th = quantile(unit_cost_bin, 0.50),
            unit_cost_90th = quantile(unit_cost_bin, 0.90),
            # find the cumulative volume produced at those percentiles
            volume_at_10th_cost = cumm_volume_produced_allwells_bin[which.min(abs(unit_cost_bin - unit_cost_10th))],
            volume_at_50th_cost = cumm_volume_produced_allwells_bin[which.min(abs(unit_cost_bin - unit_cost_50th))],
            volume_at_90th_cost = cumm_volume_produced_allwells_bin[which.min(abs(unit_cost_bin - unit_cost_90th))])

df_unit_bin_global_cumsum %>%
  group_by(depletion_limit) %>%
  summarise(total_volume = max(cumm_volume_produced_allwells_bin),
            min_cost = min(unit_cost_bin),
            max_cost = max(unit_cost_bin),
            unit_cost_10th = min_cost + 0.1 * (max_cost - min_cost),
            unit_cost_50th = min_cost + 0.5 * (max_cost - min_cost),
            unit_cost_90th = min_cost + 0.9 * (max_cost - min_cost),
            volume_at_10th_cost = cumm_volume_produced_allwells_bin[which.min(abs(unit_cost_bin - unit_cost_10th))],
            volume_at_50th_cost = cumm_volume_produced_allwells_bin[which.min(abs(unit_cost_bin - unit_cost_50th))],
            volume_at_90th_cost = cumm_volume_produced_allwells_bin[which.min(abs(unit_cost_bin - unit_cost_90th))]
  ) # %>% select(depletion_limit, total_volume, unit_cost_10th, volume_at_10th_cost, unit_cost_50th, volume_at_50th_cost, unit_cost_90th, volume_at_90th_cost)

## 1. Cost Thresholds for 25%, 50%, 75%, and 90% of Volume ----
# Calculate total volume for each depletion limit
total_volume_df <- df_unit_bin_global_cumsum %>%
  group_by(depletion_limit) %>%
  summarise(total_volume = max(cumm_volume_produced_allwells_bin))

# Merge total volume with cumsum dataframe
df_cumsum_with_total <- df_unit_bin_global_cumsum %>%
  left_join(total_volume_df, by = "depletion_limit")

# Calculate unit cost thresholds for different percentiles
percentiles <- c(0.25, 0.5, 0.75, 0.9)

cost_thresholds <- df_cumsum_with_total %>%
  group_by(depletion_limit) %>%
  summarise(across(
    percentiles,
    ~ unit_cost_bin[which.min(abs(cumm_volume_produced_allwells_bin - .x * total_volume))],
    .names = "unit_cost_{.col * 100}th"
  ))


cost_thresholds <- df_unit_bin_global_cumsum %>%
  group_by(depletion_limit) %>%
  summarise(
    total_volume = max(cumm_volume_produced_allwells_bin),
    # Use map_dfc to dynamically calculate the unit cost thresholds for each percentile
    map_dfc(
      percentiles,
      ~ tibble(!!paste0("unit_cost_", .x * 100, "th") :=
                 unit_cost_bin[which.min(abs(cumm_volume_produced_allwells_bin - .x * total_volume))])
    )
  )
...
cost_thresholds <- df_unit_bin_global_cumsum %>%
  ...
  summarise(
    total_volume = max(cumm_volume_produced_allwells_bin),
    # Use map_dfc to dynamically calculate the unit cost thresholds for each percentile
    .groups = "drop",  # To avoid retaining the group structure after summarising
    cost_thresholds = list(
      map_dfc(percentiles, function(p) {
        tibble(!!paste0("unit_cost_", p * 100, "th") :=
                 unit_cost_bin[which.min(abs(cumm_volume_produced_allwells_bin - p * total_volume))])
      })
    )
  ) %>%
  unnest(cost_thresholds)  # To unnest the list column into separate columns

print(cost_thresholds)


## 2. Gini Coefficient of Unit Costs by Depletion Limit ----
# Function to calculate the Gini coefficient
gini_coefficient <- function(x, w = rep(1, length(x))) {
  n <- length(x)
  if (n == 0) return(NA)
  x <- sort(x)
  w <- w[order(x)]
  W <- cumsum(w) / sum(w)
  X <- cumsum(w * x) / sum(w * x)
  1 - sum(diff(W) * (X[-1] + X[-length(X)]))
}

# Calculate Gini coefficient for each depletion limit
gini_df <- df_unit_bin_global_cumsum %>%
  group_by(depletion_limit) %>%
  summarise(gini = gini_coefficient(unit_cost_bin, volume_produced_allwells_bin))

print(gini_df)

## 3. Regional Disparities in Unit Water Costs ----
# TODO: do this for continents
# Calculate mean, median, and standard deviation of unit costs
summary_stats <- df_unit_bin_global_cumsum %>%
  group_by(depletion_limit) %>%
  summarise(
    mean_cost = weighted.mean(unit_cost_bin, volume_produced_allwells_bin),
    median_cost = median(unit_cost_bin),
    sd_cost = sd(unit_cost_bin)
  )

print(summary_stats)

## 4. Elasticity of Water Production to Unit Cost ----
# Calculate elasticity of water production with respect to unit cost
elasticity_df <- df_unit_bin_global_cumsum %>%
  group_by(depletion_limit) %>%
  summarise(
    elasticity = sum(volume_produced_allwells_bin * diff(log(unit_cost_bin))) /
      sum(diff(log(unit_cost_bin)) * volume_produced_allwells_bin)
  )

print(elasticity_df)

## 5. Cumulative Cost-Volume Curve (Visualization) ----
# Plot cumulative cost-volume curve
ggplot(df_cumsum_with_total, aes(x = unit_cost_bin, y = cumm_volume_produced_allwells_bin, color = depletion_limit)) +
  geom_line(size = 1) +
  labs(
    title = "Cumulative Cost-Volume Curve",
    x = "Unit Cost (USD/m³)",
    y = "Cumulative Volume Produced (10^12 m³)",
    color = "Depletion Limit"
  ) +
  theme_minimal() +
  scale_x_log10()

## 6. Temporal Trends in Unit Costs ----
# Calculate yearly average unit cost
yearly_cost_trends <- df_out %>%
  group_by(year_number) %>%
  summarise(avg_unit_cost = mean(unit_cost))

# Plot temporal trends
ggplot(yearly_cost_trends, aes(x = year_number, y = avg_unit_cost)) +
  geom_line(size = 1, color = "blue") +
  labs(
    title = "Temporal Trends in Unit Costs",
    x = "Year",
    y = "Average Unit Cost (USD/m³)"
  ) +
  theme_minimal()


## 7. Economic Feasibility Across Cost Thresholds ----
# Define cost thresholds (in USD/m³)
low_cost_threshold <- 0.1
moderate_cost_threshold <- 0.5
high_cost_threshold <- 1.0

# Classify unit costs into categories
df_feasibility <- df_unit_bin_global_cumsum %>%
  mutate(
    feasibility = case_when(
      unit_cost_bin <= low_cost_threshold ~ "Low",
      unit_cost_bin <= moderate_cost_threshold ~ "Moderate",
      unit_cost_bin <= high_cost_threshold ~ "High",
      TRUE ~ "Unaffordable"
    )
  ) %>%
  group_by(feasibility) %>%
  summarise(total_volume = sum(volume_produced_allwells_bin))

# Calculate percentage of total volume in each category
df_feasibility <- df_feasibility %>%
  mutate(percent_volume = 100 * total_volume / sum(total_volume))
